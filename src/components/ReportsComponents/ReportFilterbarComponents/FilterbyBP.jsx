...
const FilterbyBP = ({toggleDropdown , selectedOption , handleBPOptionSelect , arrowRotate , isDropdownOpen , options  }) => {
  return(
    ...
    >
        <p className="desktop:text-[15px] mobile:text-[11px] tablet:text-[12px] laptop:text-[0.9rem] laptop:leading-4 large-desktop:text-[2rem]  text-black">{selectedOption == "Clear" ? "Filter By BP" : selectedOption}</p>
        <img
            src="bp.svg"
            alt="BP Icon"
            className={`desktop:w-[20px] ${arrowRotate ? "rotate-180" : "rotate-0"} mobile:w-[18px] tablet:w-[20px] laptop:w-[16px] laptop:h-[16px] duration-300 transition-all large-desktop:w-[40px] desktop:h-[20px] large-desktop:h-[40px] pointer-events-none`}
        />
    </div>

    {/* Dropdown Menu */}
    {isDropdownOpen && (
        <div className="mt-2 absolute z-50 top-28 mobile:top-10 mobile:left-2 tablet:top-10 w-[227px] bg-[#FAFAFA] shadow-[0_4px_4px_3px_#0000000a] rounded-[15px]">
