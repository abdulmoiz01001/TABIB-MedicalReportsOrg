    ...
    return (
        <>
            <div className="flex w-full h-[18%] my-1 justify-between items-center">
                <div className="bg-[#F9B9B4] tablet:w-[50px] mobile:h-[40px] mobile:w-[40px] tablet:h-[50px] laptop:w-[50px] laptop:h-[50px] desktop:w-[62px] large-desktop:w-[17%] large-desktop:h-[150px] desktop:h-[80px] flex justify-center items-center rounded-[15px] border border-[#CC0001]">
                    ...
                    {title === 'SpO2' || title === 'Temperature' ? (
                        <div className="flex justify-start text-sm text-gray-600 mt-1">
                            <div className="flex justify-start desktop:text-[0.9rem] large-desktop:text-[1.5rem] w-full">
                                <span className="large-desktop:text-[2rem] tablet:text-[0.6rem] mobile:text-[0.6rem] laptop:text-[0.5rem]">{min}</span>~
                                <span className="large-desktop:text-[2rem] tablet:text-[0.6rem] mobile:text-[0.6rem] laptop:text-[0.5rem]">{max}</span>
                            </div>
                        </div>
                    ) : (
                        <div className="flex justify-between large-desktop:justify-between w-full desktop:text-sm text-gray-600 mt-1">
                            <span className="desktop:text-[0.9rem] laptop:text-[0.6rem] tablet:text-[0.8rem] mobile:text-[0.8rem] large-desktop:text-[2rem] text-black">{firstState}</span>
                            <span className="desktop:text-[0.9rem] laptop:text-[0.6rem] tablet:text-[0.8rem] mobile:text-[0.8rem] large-desktop:text-[2rem] text-black">{secondState}</span>
                            <span className="desktop:text-[0.9rem] laptop:text-[0.6rem] tablet:text-[0.8rem] mobile:text-[0.8rem] large-desktop:text-[2rem] text-black">{thiredState}</span>
                        </div>
                    )}
                </div>
            </div>
