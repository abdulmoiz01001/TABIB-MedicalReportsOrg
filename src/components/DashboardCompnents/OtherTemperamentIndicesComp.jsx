...
const   OtherTemperamentIndicesComp = ({reportsCounts , data}) => {
  ...
  return (
    ...
    <div className='w-full h-[50%] flex flex-col desktop:gap-0 py-2  large-desktop:gap-0 justify-start items-center h-[60%] rounded-[15px] bg-[#FAFAFA] px-2 border border-[#CC0001]' >
      <h1 className='text-[#000000] laptop:text-[0.7rem] mobile:text-[0.7rem] tablet:text-[0.7rem] desktop:text-[1rem] large-desktop:text-[1.5rem] text-center  font-bold'>Other Temperament Indices</h1>
     ...
     <ProgressBarComp reportsCounts={reportsCounts}  imgSrc="spo2.svg" title="SpO2" percentage={data.spO2.toString().slice(0, 2)} min={95} max={100} />
